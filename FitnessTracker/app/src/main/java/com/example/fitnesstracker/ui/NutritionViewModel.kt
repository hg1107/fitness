...
class NutritionViewModel(
    ...
    // API key lives in encrypted storage; exposed for the settings UI
    private val _geminiApiKey = MutableStateFlow(SecureStore.geminiApiKey)
    val geminiApiKey: StateFlow<String> = _geminiApiKey.asStateFlow()

    init {
        seedFoodDatabaseIfEmpty()
        migrateSecretsToSecureStore()
    }

    // One-time migration: move secrets out of the plaintext Room profile
    private fun migrateSecretsToSecureStore() {
        viewModelScope.launch {
            val profile = activityDao.getUserProfileSync() ?: return@launch
            if (SecureStore.geminiApiKey.isBlank() && profile.geminiApiKey.isNotBlank()) {
                SecureStore.geminiApiKey = profile.geminiApiKey
                _geminiApiKey.value = profile.geminiApiKey
            }
            if (SecureStore.mapboxToken.isBlank() && profile.mapboxToken.isNotBlank()) {
                SecureStore.mapboxToken = profile.mapboxToken
            }
            // Keep mapboxToken in the profile: it is a client-side token and the
            // map screens still read it from there. Only the Gemini key is cleared.
            if (profile.geminiApiKey.isNotBlank()) {
                activityDao.insertUserProfile(profile.copy(geminiApiKey = ""))
            }
        }
    }
    ...
    fun saveCoachSettings(
        ...
        viewModelScope.launch {
            // The API key goes to encrypted storage, never into the Room database
            SecureStore.geminiApiKey = geminiApiKey
            _geminiApiKey.value = geminiApiKey
    ...
    fun sendMessage(messageText: String) {
        ...
        viewModelScope.launch {
            ...
            // Read the key from encrypted storage (legacy fallback: Room profile)
            val activeApiKey = SecureStore.geminiApiKey.ifBlank { profile.geminiApiKey }
            ...
            if (activeApiKey.isNotBlank()) {
                try {
                    ...
                    replyText = generateOfflineCoachResponse(
                        query = messageText,
                        profile = profile.copy(geminiApiKey = activeApiKey),
                        goal = goal,
                        consumedCal = consumedCal,
                        consumedProt = consumedProt,
                        consumedCarb = consumedCarb,
